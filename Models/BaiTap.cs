...
namespace CourseWebsiteDotNet.Models
{
    public class BaiTapModel
    {
        ...
        public DateTime? thoi_han_nop { get; set; }
                ...
                using (MySqlCommand command = new MySqlCommand(query, connection))
                {
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            baiTap.thoi_han_nop = reader["thoi_han_nop"] != DBNull.Value ? Convert.ToDateTime(reader["thoi_han_nop"]) : (DateTime?)null;
                            baiTap.ngay_dang = reader["ngay_dang"] != DBNull.Value ? Convert.ToDateTime(reader["ngay_dang"]) : (DateTime?)null;
                    ...
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            return new BaiTapModel // Trả về 1 BaiTapModel
                            {
                                ...
                                thoi_han_nop = reader["thoi_han_nop"] != DBNull.Value ? Convert.ToDateTime(reader["thoi_han_nop"]) : (DateTime?)null,
        ...
        public Response InsertBaiTap(BaiTapModel baiTap)
        {
            return ExecuteDatabaseOperation(() =>
            {
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    string query = "INSERT INTO bai_tap (thoi_han_nop, ngay_dang, ten, noi_dung, thoi_han, id_giang_vien, id_muc) " +
                                    "VALUES (@thoi_han_nop, @ngay_dang, @ten, @noi_dung, @thoi_han, @id_giang_vien, @id_muc); " +
                                    "SELECT LAST_INSERT_ID();";

                    using (MySqlCommand command = new MySqlCommand(query, connection))
                    {
                        command.Parameters.AddWithValue("@thoi_han_nop", baiTap.thoi_han_nop);
                        command.Parameters.AddWithValue("@ngay_dang", baiTap.ngay_dang);
